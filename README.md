...
- At the root of the cloned `pausable-udt` repository, copy paste the following content (as you may see in `.env.example`) into a `.env` file, or you can just run `ccc-deploy deploy pausable_udt_script` to automatically create one and then fill in the missing fields. Usually you only need to fill in `WALLET_PRIVATE_KEY`.

```env
WALLET_PRIVATE_KEY=c0f0166fbbabca125cd8f9e12ddde5bc204fcbdbb090d6c1eb3a29bc02802999
CKB_RPC_URL=wss://testnet.ckb.dev/ws
SSRI_SERVER_URL=http://localhost:9090
```

At the same location, run:

```shell
ccc-deploy deploy pausable_udt_script --forceNew --privateKey c0f0166fbbabca125cd8f9e12ddde5bc204fcbdbb090d6c1eb3a29bc02802999
```
